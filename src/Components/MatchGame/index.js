...
class MatchGame extends Component {
  ...
  imgClick = id => {
    const {randomObj} = this.state
    if (randomObj.id === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomObj:
          imagesList[Math.floor(Math.floor(Math.random() * 100) / 3.5)],
      }))
    } else {
      this.clearTimer()
      this.setState({
  ...
  render() {
    const {activeTabId, randomObj, scoreCard} = this.state
    const activeList = imagesList.filter(each => each.category === activeTabId)
    const timeUp = this.checkTimer()
    console.log(Math.floor(Math.floor(Math.random() * 100) / 3.5))
    return (
      <div>
        <Navbar navDetails={this.state} />
          ...
          {scoreCard && timeUp ? (
            <>
              <img
                ...
                alt="match"
                className="match-img-css"
              />
              <ul className="tabs-container">
                {tabsList.map(eachTab => (
                  <TabItem
                    tabDetails={eachTab}
                    activeCheck={eachTab.tabId === activeTabId}
                    key={eachTab.tabId}
            ...
            <ScoreCard scoreDetails={this.state} resetClick={this.resetClick} />
          )}
        </div>
      </div>
    )
